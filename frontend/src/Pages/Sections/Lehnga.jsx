    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          const filtered = data.filter(p => (p.category || '').toLowerCase() === 'lehnga');
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto mt-8 px-4">
        <h2 className="text-2xl font-bold text-rose-500 mb-6 text-center">Lehngas</h2>
        <div className="flex flex-row items-center mb-6 px-2 w-full">
          <input
            type="text"
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search lehnga..."
